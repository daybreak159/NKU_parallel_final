...
public:
    ...
    // 辅助函数，便于类型检查
    inline bool isLetter() const { return type == 1; }
    inline bool isDigit() const { return type == 2; }
    inline bool isSymbol() const { return type == 3; }
};
...
public:
    ...
    void init();
    void Generate(PT pt);
    void GenerateParallelOMP(PT pt, int num_threads = -1);
    void PopNext(int num_threads = -1);
    int total_guesses = 0;
    vector<string> guesses;
    
private:
    segment* getSegmentPtr(const segment& seg);
    static const int PARALLEL_TASK_MIN = 500;
};