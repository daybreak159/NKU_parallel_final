...
#include <iomanip>  // 为了使用setfill和setw
...
class MD5 {
public:
    std::string GetMD5HashString(const std::string& input) {
        bit32 state[4];
        MD5Hash(input, state);
        return convertToHex(state);
    }

private:
    std::string convertToHex(bit32 state[4]) {
        std::stringstream ss;
        ss << std::hex << std::setfill('0');
        
        for (int i = 0; i < 4; i++) {
            bit32 a = state[i];
            ss << std::setw(2) << ((a >>  0) & 0xff)
               << std::setw(2) << ((a >>  8) & 0xff)
               << std::setw(2) << ((a >> 16) & 0xff)
               << std::setw(2) << ((a >> 24) & 0xff);
        }
